...
export default class SFDX {
	...
	async RunShell({ config, data }) {
		ET_Asserts.hasData({ value: config, message: 'config' });
		ET_Asserts.hasData({ value: data, message: 'data' });
		ET_Asserts.hasData({ value: data.app, message: 'data.app' });
		// ET_Asserts.hasData({ value: data.params, message: 'data.params' });
		let logFile, command;

		const { stepNumber, stepMethod } = this.getStepId({ config });
		config.currentStep = `${stepNumber}. ${stepMethod}`;
		command = `${data.app} ${data.params ? data.params : ''}`.trim();
		logFile = `${stepNumber}_${stepMethod}.json`;
		await this._runSFDX({ config, command, logFile });
	}
	...
	async GeneratePassword({ config }) {
		...
		const displayPassword = async () => {
			...
			if (result.CLOSE.code === 0) {
				...
				Colors2.sfdxShowNote({
					msg: JSON.stringify(
						{
							username: obj.user.username,
							domain: obj.user.instanceUrl,
						},
						null,
						2,
					),
		...
		const updateMetadataWithUsername = async () => {
			let path;

			// Set environment variable for create script
			path = `${config.rootLogs}/_user.json`;
			const _user = JSON.parse(await OS2.readFile({ config, path }));
			const username = _user.user.username;
			process.env.NEW_USERNAME = username;

			// Set script to build after that
			path = `${config.rootLogs}/../build.sh`;
			const buildFile = await OS2.readFile({ config, path });

			// Update the NEW_USERNAME line
			const updatedData = buildFile.replace(/(export NEW_USERNAME=")([^"]*)(")/, `$1${username}$3`);

			await OS2.writeFile({ config, path, data: updatedData });
		};

		await createPassword();
		await displayPassword();
		await updateMetadataWithUsername();
	}
	...
	async ShowFinalSuccess({ config }) {
		...
		if (config.errors.length > 0) {
			...
			const displayUser = async () => {
				...
				let result = await this._runButNotLog({ config, command });
				if (result.CLOSE.code === 0) {
					let stdOut = JSON.parse(result.STDOUT);
					let user = stdOut.result;
					Colors2.sfdxShowSuccess({
						msg: JSON.stringify(
							{
								username: user.username,
								domain: user.instanceUrl,
							},
							null,
							2,
						),
	...
	async _runButNotLog({ config, command }) {
		ET_Asserts.hasData({ value: config, message: 'config' });
		ET_Asserts.hasData({ value: command, message: 'command' });

		return await this._runAndLog({ config, command, logFile: 'SKIP' });
	}
